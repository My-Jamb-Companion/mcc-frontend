...
export default function CourseHeroCard({
  ...
  return (
    <div
      onClick={onClick}
      className="flex gap-6 cursor-pointer group w-full max-md:flex-col max-md:gap-4"
    >
      <div className="w-full max-h-70 max-w-95 max-md:max-w-full max-md:max-h-40 rounded-2xl overflow-hidden shrink-0 relative">
        <Image
          src={image}
          alt={title}
          width={100}
          ...
          <div className="flex items-center gap-4 border border-gray-200 rounded-xl flex-1">
            {isPremium && (
              <div className="flex flex-col items-center justify-center gap-1 bg-primary text-white rounded-tl-xl rounded-bl-xl px-6 py-3.5 shrink-0">
                <Icon icon="bitcoin-icons:verify-outline" size={23} />
