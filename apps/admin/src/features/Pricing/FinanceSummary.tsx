...
export default function FinanceSummary() {
  ...
  const input = "rounded-lg border border-neutral-200 px-3 py-2 text-sm outline-none focus:border-violet-500";
    ...
    <section className="flex flex-col gap-6 pb-10">
      <div className="flex flex-col gap-4">
        <div>
          ...
          <h1 className="mt-2 text-2xl font-bold text-neutral-900">Finance summary</h1>
      ...
      <div className="flex flex-wrap items-end gap-3 rounded-2xl border border-neutral-100 bg-white p-5 shadow-sm">
        <div className="flex flex-col gap-1">
          <label htmlFor="finance-from" className="text-xs font-medium text-neutral-500">From</label>
          <input id="finance-from" type="date" value={range[0]} max={range[1]} className={input}
            onChange={(e) => setRange([e.target.value, range[1]])} />
        </div>
        <div className="flex flex-col gap-1">
          <label htmlFor="finance-to" className="text-xs font-medium text-neutral-500">To</label>
        ...
        {!valid && <p className="basis-full text-xs text-red-600">Choose a start date on or before the end date.</p>}
      </div>

      {isLoading ? (
        <p className="text-sm text-neutral-400">Adding up the period…</p>
      ) : isError || !data ? (
        valid && (
        ...
        <div className={`flex flex-col gap-6 transition-opacity ${isFetching ? "opacity-60" : ""}`}>
          ...
          <div className="rounded-2xl border border-neutral-100 bg-white p-6 shadow-sm">
            <h2 className="text-base font-bold text-neutral-900">Published-price sales</h2>
            <p className="mt-1 text-sm text-neutral-500">Each checkout&apos;s recorded split of what the student paid.</p>
                ...
                <tbody className="divide-y divide-neutral-50">
                  {rows.map((r) => (
                    <tr key={r.key} className={r.key === "amount" || r.key === "profit_before_tax" ? "font-semibold" : ""}>
                      <td className="py-2 pr-3 text-neutral-800">{r.label}</td>
                      <td className="py-2 pr-3 text-right tabular-nums text-neutral-700">{naira(r.sales)}</td>
                      <td className="py-2 pr-3 text-right tabular-nums text-neutral-500">{r.refunds ? naira(-r.refunds) : "—"}</td>
          ...
          <div className="grid gap-4 lg:grid-cols-2">
            <div className="rounded-2xl border border-neutral-100 bg-white p-6 shadow-sm">
              <h2 className="text-base font-bold text-neutral-900">Sales at old flat prices</h2>
            ...
            <div className="rounded-2xl border border-neutral-100 bg-white p-6 shadow-sm">
              <h2 className="text-base font-bold text-neutral-900">Gems purchases</h2>
              <p className="mt-1 text-sm text-neutral-500">Gem top-ups aren&apos;t priced by the pricing model, so they have no breakdown.</p>
